...
import com.rohini.service.ProductService;
import com.rohini.utility.DBUtil;
import com.rohini.utility.IDUtil;
...
public class ProductServiceImpl implements ProductService {
	...
	public String addProduct(String prodName, int prodType, String prodInfo, double prodPrice, int prodQuantity,
		...
		ProductCreationBean product = new ProductCreationBean(prodId, prodName, prodType, prodInfo, prodPrice, prodQuantity, prodImage);
	...
	public String addProduct(ProductCreationBean product) {
		...
		try {
			...
			ps.setInt(3, product.getProdType());
	...
	public String updateProduct(ProductCreationBean prevProduct, ProductCreationBean updatedProduct) {
		...
		try {
			...
			ps.setInt(2, updatedProduct.getProdType());
	...
	public List<ProductBean> getAllProducts() {
		...
		try {
			ps = con.prepareStatement("select product.pid, product.pname,category.name as ptname, product.ptype, product.pinfo, product.pprice, product.pquantity, product.image from product LEFT JOIN category ON category.id=product.ptype");
			...
			while (rs.next()) {
				...
				product.setProdTName(rs.getString(3));
				product.setProdType(rs.getInt(4));
	...
	public List<ProductBean> getAllProductsByType(int type) {
		...
		try {
			String query = "SELECT product.pid, product.pname, category.name as ptname, product.ptype, product.pinfo, product.pprice, product.pquantity, product.image FROM product " +
		               "LEFT JOIN category ON category.id=product.ptype " +
		               "WHERE product.ptype =" + type + ";";
	...
	public List<ProductBean> getAllProductsByTypeLimitFour(int type) {
		...
		try {
			String query = "SELECT product.pid, product.pname, category.name as ptname, product.ptype, product.pinfo, product.pprice, product.pquantity, product.image FROM product " +
		               "LEFT JOIN category ON category.id=product.ptype " +
		               "WHERE product.ptype =" + type + " LIMIT 4;";
	...
	public List<ProductBean> searchAllProducts(String search) {
		...
		try {
			...
			while (rs.next()) {
				...
				product.setProdId(rs.getString(1));
				product.setProdName(rs.getString(2));
				product.setProdType(rs.getInt(3));
	...
	public ProductBean getProductDetails(String prodId) {
		...
		try {
			...
			String query = "SELECT product.pid, product.pname, category.name as ptname, product.ptype, product.pinfo, product.pprice, product.pquantity, product.image  FROM `shopping-cart`.product "
					+ "LEFT JOIN category ON category.id=product.ptype"
